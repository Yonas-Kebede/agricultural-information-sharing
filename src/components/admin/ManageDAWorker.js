...
function ManageDAWorker() {
  ...
  const fetchData = async (page) => {
    setIsloading(true);
    try {
      const response = await axios.get(`http://localhost:3001/user/getallusers?page=${page}&limit=4&role=${roles}`);
  ...
  return (
    ...
    <div >
      ...
      <Paper elevation={0} sx={{ display: { xs: 'block', sm: 'flex' }, fontSize: { xs: '10px', sm: '20px' }, marginTop: {xs:'5rem',md:'3rem '}  }}>
        <Paper  sx={{ display: 'block', margin: '3% 0% auto 1%', backgroundColor: 'whitesmoke',maxHeight: '33rem', width:{xs:'90%',md:'24%'}  }}>
        ...
        <Paper sx={{ backgroundColor: 'whitesmoke', margin: '3% 1% 1%', width: {xs:'90%',md:'74%' }}}>
